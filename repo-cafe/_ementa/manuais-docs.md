...
-   **11.2. Manual de Padrões de Arquitetura Avançados:**
    -   **Descrição:** Uma expansão do conhecimento de arquitetura para além da Máquina de Estados. Este manual introduziria e compararia outros padrões poderosos:
        ...
        -   **Injeção de Dependência (Dependency Injection):** Uma técnica para desacoplar ainda mais os sistemas, facilitando a substituição de módulos (ex: trocar o `SaveSystem` local por um na nuvem) e os testes unitários.
